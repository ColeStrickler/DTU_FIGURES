...
# Load + clean
# -----------------------------
df = pd.read_csv("data/slicing_rocket1.0.csv", skipinitialspace=True)
...
# Extract image size
# -----------------------------
#sizes = df["benchmark"].str.extract(r'(\d+)x(\d+)x(\d+)x(\d+)')
#print(sizes)
...
def plot_ax(ax, pivot_mean, index, xlabel,ylabel):
    ...
    ax.set_ylim(0.0, 5.5)         # set lower and upper limits
    ...
    ax.set_title(f"Tensor Slicing {size}", fontsize=12, fontweight="bold")
...
for ax, size in zip(axes, unique_sizes):
   ...
   # print(grouped)
    ...
    pivot_mean["base_inplace"] = pivot_mean["inplace"] / pivot_mean["dtu"]
    ...
    # Keep the benchmark order sorted by stride
   ...
   #ax.set_yscale("log", base=2)
    ...
    if size != '1':
        batch_labels = [  (size + ":" + str(val)) for val in batch_labels]
    print(batch_labels)

    plot_ax(ax, pivot_mean, batch_labels, " Element Stride", "Normalized Exec. Time")
    label_total_bar(ax)
...
plt.savefig("figures/slicing_rocket.png", bbox_inches="tight")
plt.savefig("figures/slicing_rocket.pdf", bbox_inches="tight")
plt.show()
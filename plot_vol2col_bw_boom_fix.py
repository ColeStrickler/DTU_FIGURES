...
# Load + clean
# -----------------------------
df = pd.read_csv("data/fix_vol2col_boom3.2.csv", skipinitialspace=True)
...
def plot_ax(ax, pivot_mean, index, xlabel,ylabel):
    ...
    ax.set_title(f"Vol2Col Total Memory Accesses {size}", fontsize=12, fontweight="bold")
...
for ax, size in zip(axes, unique_sizes):
   ...
   # transform_mean = grouped.pivot(index="benchmark", columns="type", values="transform_mean")
    ...
    plot_ax(ax, pivot_mean, batch_labels, "Kernel Size", "Normalized # DRAM Accesses")
    label_total_bar(ax)
...
total_df = pd.read_csv("data/fix_avg_memory_traffic_boom.csv")
...
# Save back
df.to_csv("data/fix_avg_memory_traffic_boom.csv", index=False)


plt.savefig("figures/fix_vol2col_bw_boom.png", bbox_inches="tight")
plt.savefig("figures/fix_vol2col_bw_boom.pdf", bbox_inches="tight")
plt.show()
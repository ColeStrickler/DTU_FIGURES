...
\ifcase#1 c4\or c3\or c2\or c1\or c0\fi
}
...
\newcommand{\rotzright}[3]{%
  % input: x y z
  % output: (y, DX-1-x, z)
  \edef\tx{#2}% new X = old Y
  \edef\ty{\number\numexpr\DX-1-#1\relax}% new Y = DX-1 - old X
  \edef\tz{#3}% Z unchanged  
}


\newcommand{\rotshaperight}[3]{
  % input: #1=x, #2=y, #3=z
  % output: rotated indices in \outx, \outy, \outz

  \edef\outx{#2}% X unchanged
  \edef\outy{#1}% new Y = old Z
  \edef\outz{#3}% new Z = old Y
}

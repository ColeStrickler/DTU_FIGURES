...
# Load + clean
# -----------------------------
df = pd.read_csv("data/unfold_rocket1.0.csv", skipinitialspace=True)
...
def plot_ax(ax, pivot_mean, index, xlabel,ylabel):
    ...
    ax.set_title(f"Tensor Size: {size}MB", fontsize=12, fontweight="bold")
...
for ax, size in zip(axes, unique_sizes):
   ...
   #ax.set_yscale("log", base=2)
    ...
    plot_ax(ax, pivot_mean, batch_labels, "Mode", "Normalized Exec. Time")
    label_total_bar(ax)
...
plt.savefig("figures/unfold_rocket.png", bbox_inches="tight")
plt.savefig("figures/unfold_rocket.pdf", bbox_inches="tight")
plt.show()
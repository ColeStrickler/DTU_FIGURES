...
# Load + clean
# -----------------------------
df = pd.read_csv("data/fix_im2col_boom3.2.csv", skipinitialspace=True)
#print(df)
...
def plot_ax(ax, pivot_mean, index, xlabel,ylabel):
    ...
    ax.set_title(f"Im2Col Total Memory Accesses {size}", fontsize=12, fontweight="bold")
...
plt.savefig("figures/fix_im2col_bw_boom.png", bbox_inches="tight")
plt.savefig("figures/fix_im2col_bw_boom.pdf", bbox_inches="tight")
plt.show()